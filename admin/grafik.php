...
<html>
    ...
    <div class="form-panel">
        <h2>List Data Grafik</h2>
        <table border="1" class="table table-striped table-advance table-hover">
            <tr><th>NO</th><th>Jabatan</th><th>Total Pegawai</th><th>Act</th></tr>
            <?php
            include 'conn.php';
            $mahasiswa = mysqli_query($conn, "SELECT * from tbl_grafik");
            $no = 1;
            foreach ($mahasiswa as $row) {
                // $jenis_kelamin = $row['jabatan'] == 'P' ? 'Perempuan' : 'Laki laki';
                 echo "<tr>
            <td>$no</td>
            <td>" . $row['posisi'] . "</td>
            <td>" . $row['total'] . "</td>
          
            <td><a href='grafik2.php?id_grafik=$row[id_grafik]'><button>Edit</button></a>
